...
import pickNotionProperties from '$shared/lib/pickNotionProperties';
import notion from '$shared/middleware/notion';
import { type NotionPage, type NotionPageKind } from '$shared/types/notion';
 ...
 * @param kind 속성을 검사할 발행 스키마 (`NOTION_PROPERTY_SCHEMA`)
 * @returns 속성을 스키마대로 좁힌 페이지
 ...
 */
export default async function getNotionPage<K extends NotionPageKind>(
  pageId: string,
  ...
  return { ...page, properties: pickNotionProperties(page.properties, kind) };
}
