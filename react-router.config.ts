 ...
 *
 * v2 설정 파일의 `ignoredRouteFiles` 는 여기 없다. React Router v7 은 `routes.ts` 로 라우트를
 * 명시하거나 `@react-router/fs-routes` 로 파일 규약을 쓰는데, 이 레포는 `app/routes/` 의
 * flat routes 파일 규약을 v2 때부터 그대로 쓰므로 `fs-routes` 를 붙였다.
 *
